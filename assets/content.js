!(function( host ){
     ...
     * 是否开启划词翻译
     */
    var IF_TRANSLATE_SELECTION = true;
    ...
    ICIBA.getSettings(function( settings ){
        ...
        chrome.storage.onChanged.addListener(function callback( changes ){
            ...
            if( setting_huaci ){
                IF_TRANSLATE_SELECTION = true;
            }
        });
    });

    $('.J_Iciba').on('mousedown',function(e){
        e.stopPropagation();
    });
    ...
    var getPosition = function( refer, tooltip, container ){
        ...
        console.log((scrollLeft + winWidth ) - ( referX + referW+tooltipW ))
        // 左边比较宽
        if(( scrollLeft + winWidth ) - ( referX + referW+tooltipW )<0){
